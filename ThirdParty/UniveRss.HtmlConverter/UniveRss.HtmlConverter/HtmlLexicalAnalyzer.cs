...
namespace UniveRss.HtmlConverter
{
  ...
  internal class HtmlLexicalAnalyzer
  {
        ...
        if (_nextCharacter == '<' && (_lookAheadCharacter == '?' || _lookAheadCharacter == '!'))
        {
          ...
          if (_lookAheadCharacter == '[')
          {
            ...
            while (!IsAtEndOfStream &&
                   !(_previousCharacter == ']' && _nextCharacter == ']' && _lookAheadCharacter == '>'))
            {
              GetNextCharacter();
            }
            if (_nextCharacter == '>')
    ...
    private bool IsAtTagStart => _nextCharacter == '<' && (_lookAheadCharacter == '/' || IsGoodForNameStart(_lookAheadCharacter)) &&
                                 !_isNextCharacterEntity;

    private bool IsAtTagEnd => (_nextCharacter == '>' || (_nextCharacter == '/' && _lookAheadCharacter == '>')) &&
                               !_isNextCharacterEntity;
