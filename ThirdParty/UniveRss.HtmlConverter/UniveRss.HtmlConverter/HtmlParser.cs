...
namespace UniveRss.HtmlConverter
{
  ...
  internal class HtmlParser
  {
    ...
    internal static string ExtractHtmlFromClipboardData(string htmlDataString)
    {
      ...
      startHtmlIndex = Int32.Parse(htmlDataString.Substring(startHtmlIndex + "StartHTML:".Length,
                                                            "0123456789".Length));
      ...
      // TODO: We assume that indices represented by strictly 10 zeros ("0123456789".Length),
      // which could be wrong assumption. We need to implement more flrxible parsing here
      endHtmlIndex = Int32.Parse(htmlDataString.Substring(endHtmlIndex + "EndHTML:".Length,
                                                          "0123456789".Length));
      ...
      return htmlDataString.Substring(startHtmlIndex,
                                      endHtmlIndex - startHtmlIndex);
    }
    ...
    internal static string AddHtmlClipboardHeader(string htmlString)
    {
      ...
      int startFragment = htmlString.IndexOf(HtmlStartFragmentComment,
                                             0);
      if (startFragment >= 0)
      {
      ...
      int endFragment = htmlString.IndexOf(HtmlEndFragmentComment,
                                           0);
      if (endFragment >= 0)
      {
      ...
      stringBuilder.AppendFormat(HtmlHeader,
                                 startHTML,
                                 endHTML,
                                 startFragment,
                                 endFragment,
    ...
    private void InvariantAssert(bool condition,
                                 string message)
    {
      if (!condition)
      {
    ...
    private XmlElement ParseHtmlContent()
    {
      ...
      XmlElement htmlRootElement = _document.CreateElement("html",
                                                           XhtmlNamespace);
    ...
    {
      XmlElement htmlElementCopy = _document.CreateElement(htmlElement.LocalName,
                                                           XhtmlNamespace);
      for (int i = 0; i < htmlElement.Attributes.Count; i++)
      {
        XmlAttribute attribute = htmlElement.Attributes[i];
        htmlElementCopy.SetAttribute(attribute.Name,
                                     attribute.Value);
      }
    ...
    private void AddEmptyElement(XmlElement htmlEmptyElement)
    {
      ...
      // if empty element is img and there is <a> element in the pending elements
      // add img as a child of a element
      if (htmlEmptyElement.Name.ToLower() == "img" && _pendingInlineElements.Count > 0)
      {
        XmlElement pending = _pendingInlineElements.Peek();
        if (pending.Name.ToLower() == "a")
        {
          pending.AppendChild(htmlEmptyElement);
        }
      }
      else
      {
        // add empty element as a child of parent element
    ...
    private void OpenStructuringElement(XmlElement htmlElement)
    {
      ...
      if (_openedElements.Count > 0)
      {
        ...
        if (HtmlSchema.ClosesOnNextElementStart(htmlParent.LocalName,
    ...
    private void ParseAttributes(XmlElement xmlElement)
    {
             ...
             _htmlLexicalAnalyzer.NextTokenType != HtmlTokenType.EmptyTagEnd)
      {
        ...
        if (_htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.Name)
        {
          ...
          // TODO tpa hack
          if (attributeName.TrimEnd().EndsWith(":"))
