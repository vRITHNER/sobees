...
namespace UniveRss.HtmlConverter
{
  ...
  internal static class HtmlFromXamlConverter
  {
    ...
    public static string ConvertXamlToHtml(string xamlString)
    {
      ...
      if (!WriteFlowDocument(xamlReader,
                             htmlWriter))
      {
        return "";
      }
    ...
    /// </param>
    private static bool WriteFlowDocument(XmlTextReader xamlReader,
                                          XmlTextWriter htmlWriter)
    {
      ...
      WriteFormattingProperties(xamlReader,
                                htmlWriter,
                                inlineStyle);

      WriteElementContent(xamlReader,
                          htmlWriter,
                          inlineStyle);
    ...
    /// </param>
    private static void WriteFormattingProperties(XmlTextReader xamlReader,
                                                  XmlTextWriter htmlWriter,
      ...
      while (xamlReader.MoveToNextAttribute())
      {
        ...
        switch (xamlReader.Name)
        {
          ...
          case "Width":
            ...
            break;
          case "ColumnSpan":
            htmlWriter.WriteAttributeString("COLSPAN",
                                            xamlReader.Value);
            break;
          case "RowSpan":
            htmlWriter.WriteAttributeString("ROWSPAN",
                                            xamlReader.Value);
            break;
        }
                                            ...
                                            StringBuilder inlineStyle)
    {
      ...
      if (xamlReader.IsEmptyElement)
      {
        if (htmlWriter != null && !elementContentStarted && inlineStyle.Length > 0)
        {
          ...
          htmlWriter.WriteAttributeString("STYLE",
    ...
    /// </param>
    private static void AddComplexProperty(XmlTextReader xamlReader,
                                           StringBuilder inlineStyle)
    {
      //Debug.Assert(xamlReader.NodeType == XmlNodeType.Element);
    ...
    /// </param>
    private static void WriteElement(XmlTextReader xamlReader,
                                     XmlTextWriter htmlWriter,
                                     StringBuilder inlineStyle)
    {
      //Debug.Assert(xamlReader.NodeType == XmlNodeType.Element);
      ...
      if (htmlWriter == null)
      {
        ...
        WriteElementContent(xamlReader, /*htmlWriter:*/null,
                            null);
      }
      else
      {
